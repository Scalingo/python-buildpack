...
function pip::install_dependencies() {
	...
	# "Requirement already satisfied: typing-extensions==4.12.2 in ./.scalingo/python/lib/python3.10/site-packages (from -r requirements.txt (line 2)) (4.12.2)"
	# ...when using Python 3.11+:
	# "Requirement already satisfied: typing-extensions==4.12.2 in /app/.scalingo/python/lib/python3.13/site-packages (from -r requirements.txt (line 5)) (4.12.2)"
	# shellcheck disable=SC2310 # This function is invoked in an 'if' condition so set -e will be disabled.
	if ! {
		"${pip_install_command[@]}" \
			--disable-pip-version-check \
