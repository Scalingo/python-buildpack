...
function pip::install_dependencies() {
	...
	set +e
	# shellcheck disable=SC2154 # TODO: Env var is referenced but not assigned.
	/app/.scalingo/python/bin/pip install "${args[@]}" --exists-action=w --src='/app/.scalingo/src' --disable-pip-version-check --no-cache-dir --progress-bar off 2>&1 | tee "${WARNINGS_LOG}" | cleanup | output::indent
	...
	if [[ "${INSTALL_TEST:-0}" == "1" ]]; then
		if [[ -f requirements-test.txt ]]; then
			...
			/app/.scalingo/python/bin/pip install -r requirements-test.txt --exists-action=w --src='/app/.scalingo/src' --disable-pip-version-check --no-cache-dir 2>&1 | cleanup | output::indent
		fi
	fi
}
