...
function pipenv::install_dependencies() {
	...
	if [[ "${INSTALL_TEST:-0}" == "1" ]]; then
		...
		/app/.scalingo/python/bin/pipenv install --dev --system --deploy --extra-pip-args='--src=/app/.scalingo/src' 2>&1 | cleanup | output::indent

	# Install the dependencies.
	elif [[ ! -f Pipfile.lock ]]; then
		output::step "Installing dependencies with Pipenv"
		/app/.scalingo/python/bin/pipenv install --system --skip-lock --extra-pip-args='--src=/app/.scalingo/src' 2>&1 | output::indent

	else
		output::step "Installing dependencies with Pipenv"
		/app/.scalingo/python/bin/pipenv install --system --deploy --extra-pip-args='--src=/app/.scalingo/src' 2>&1 | output::indent
	fi
}
