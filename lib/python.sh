...
DEFAULT_S3_BASE_URL="https://python-binaries.scalingo.com"
...
function python::install() {
	...
	local install_dir="${build_dir}/.scalingo/python"
		...
		if ! {
			...
			if [[ "${python_full_version}" != "${latest_known_patch_version}" ]] && grep --quiet "The requested URL returned error: 40[34]" "${error_log}"; then
				output::error <<-EOF
					...
					of ${python_full_version}, however, we couldn't find that version.
